...
class WithdrawVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        withdrawTable.dataSource = self
        withdrawTable.delegate = self
...
extension WithdrawVC: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let vc = storyboard?.instantiateViewController(identifier: "WithdrawDetails") as? WithdrawDetailsVC
        
        vc?.cellWithdraw = currentCell.index
        
        self.present(vc!, animated: true)
        
        //        self.show(vc!, sender: nil)
        
        //        vc?.navigationItem.title = "Withdraw Request"
